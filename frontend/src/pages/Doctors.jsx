...
const specialities = ['General Physician', 'Gynecologist', 'Dermatologist', 'Pediatrician', 'Neurologist']
...
const Doctors = () => {
  ...
  // Filter by speciality and search term
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-indigo-50 px-4 sm:px-10 py-10 mb-16">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        {/* Sidebar filter */}
        <aside className="md:w-64 md:sticky md:top-20 self-start bg-white rounded-xl shadow-md p-6 mb-8 md:mb-0">
          ...
          <ul className="flex flex-col gap-3">
            {specialities.map(cat => {
              ...
              return (
                <li
                  key={cat}
                  onClick={() => (active ? navigate('/doctors') : navigate(`/doctors/${cat}`))}
